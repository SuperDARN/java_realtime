...
public class DataMap {
   ...
   public boolean decode(byte buf[],int off)  {
       ...
       off+=4; /* skip the header */

       rsize=MathConvert.lng(buf,off); off+=4;
	   ...
	   while ((off+n<size) && (buf[off+n] !=0)) n++;
           if (off+n>=size) break;
	 ...
	 while ((off+n<size) && (buf[off+n] !=0)) n++;
         ...
         System.err.println("<"+name+"> num of dims:"+dim);
         off+=4;
         rng=new int[dim];
         t=1;
         for (n=0;n<dim;n++) {
